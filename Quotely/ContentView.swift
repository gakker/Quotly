...
struct ContentView: View {
    ...
    let imageCachedHelper = ImageCachedHelper()
    ...
    var body: some View {
        ZStack {
                ...
                TabView {
                    ForEach(images, id: \.self) { image in
                        AsyncImage(url: URL(string: image)) { image in
                            image.resizable().frame(width: proxy.size.width, height: proxy.size.height)
            ...
            .ignoresSafeArea().onAppear{
                ...
                AppUtils.getDataFromUrl(url: imageUrl!) { data, response, error in
                    if data != nil {
                        print("Saving Value")
                        let imageCachedHelper = ImageCachedHelper()
                        imageCachedHelper.saveValue(key: "myKey", value: data!)
                    }
                }
            }
    }
...
struct TabBar: View {
    var body: some View {
        HStack(spacing: 32) {
            MTabBarIcon(icon: "photo.on.rectangle.angled", isActive: true)
            MTabBarIcon(icon: "person.crop.circle")
        }
